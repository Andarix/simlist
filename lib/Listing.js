...
var valid_fields = {
    ...
    "sid": shortString                       // unique server id
};

exports.Listing = function Listing (dns, port, sid) {
    this.port = port;
    this.dns = dns;
    this.sid = sid;
...
exports.Listing.prototype.update_from_body = function (from_body) {
    for (key in valid_fields) {
        if (valid_fields.hasOwnProperty(key) && (key in from_body)) {
...
exports.validate_dns = function (value, alt_value, reqip, success, failure) {
    ...
    if (reqip.match(/::ffff:/)) {
    	// on dualstack we may get mixed syntax "::ffff:IP number" for IPv4
    	reqip = reqip.substr(7);
    }

    // DNS MUST be valid FQDN, IPv4 or IPv6
    // since dualstack machines prefer IP6, lets check first if one of the dns strings is IP6
    if (value === reqip  ||  alt_value === reqip) {
        return success();
    }
    ...
    if (validator.isIP(value, "4")) {
        console.error("dns.validate - failure (valid IPv4 %s but does not match request IP %s)",reqip,value);
    }

    if (checkdomain(value)) {
        console.error("IP %s, alt IP %s",value,alt_value);
        dns.resolve6(value, function (err, addresses) {
        console.error("validate6 %s, %s, %s", value, err, addresses);
            ...
            dns.resolve4(value, function (err, addresses) {
                //console.error("validate4 %s, %s, %s", value, err, addresses);
                if (reqip.match(/::ffff:/)) {
                    // somehow we get strings like "::ffff:IP number" for IPv4
                    reqip = reqip.substr(7);
                }
                if (!err && addresses.length > 0) {
                    var found = false;
                ...
                console.error("dns.validate - failure (no matching addresses for %s in DNS): %s",value,err);
                return success();
                //return failure();
            });
        });
